...
const items = computed(() =>
    props.teachers.map((t) => ({
        ...t,
        email: t.user?.email ?? '—',
        full_name: [t.first_name, t.last_name].filter(Boolean).join(' ') || t.user?.name || '—',
        status_label: statusLabel(t.status),
        deleted_display: formatDeleted(t.deleted_at),
    })),
);
...
function openRestoreDialog() {
    if (selectedIds.value.length === 0) {
        showWarning('Zgjidh të paktën një mësimdhënës me checkbox për ta rikthyer.');
        return;
    }
...
function openForceDeleteDialog() {
    if (selectedIds.value.length === 0) {
        showWarning('Zgjidh të paktën një mësimdhënës me checkbox për ta fshirë përgjithmonë.');
        return;
    }
    ...
    <TenantLayout>
        ...
        <v-dialog v-model="restoreConfirmOpen" max-width="440" persistent>
            <v-card rounded="lg">
                <v-card-title class="d-flex align-center ga-2 text-h6">
                    ...
                    Rikthe mësimdhënësit?
                </v-card-title>
                <v-card-text class="text-body-1 text-medium-emphasis">
                    <template v-if="selectedIds.length === 1">
                        Ky mësimdhënës do të rikthehet te lista e mësimdhënësve aktivë.
                    </template>
                    <template v-else>
                        {{ selectedIds.length }} mësimdhënës do të rikthehen te lista e mësimdhënësve aktivë.
                    </template>
                </v-card-text>
        ...
        <v-dialog v-model="forceDeleteConfirmOpen" max-width="440" persistent>
            <v-card rounded="lg">
                ...
                <v-card-text class="text-body-1 text-medium-emphasis">
                    <template v-if="selectedIds.length === 1">
                        Ky mësimdhënës dhe llogaria e përdoruesit do të fshihen përgjithmonë. Ky veprim nuk mund të zhbëhet.
                    </template>
                    <template v-else>
                        {{ selectedIds.length }} mësimdhënës do të fshihen përgjithmonë bashkë me llogaritë e tyre. Ky veprim
                        nuk mund të zhbëhet.
                    </template>
                </v-card-text>
                <v-card-actions class="px-4 pb-4">
        ...
        <v-card class="overflow-hidden" rounded="lg">
            <v-toolbar color="primary" density="comfortable" flat>
                <Link class="d-inline-flex text-decoration-none" href="/teachers">
                    <v-btn color="white" icon="mdi-arrow-left" variant="text" />
                </Link>
                <v-toolbar-title class="text-white"> Teacher archive (trash) </v-toolbar-title>
                <v-spacer />

                <v-tooltip location="bottom" text="Rikthe mësimdhënës të zgjedhur (restore)">
                ...
                <v-tooltip location="bottom" text="Fshi përgjithmonë mësimdhënës të zgjedhur">
                    <template #activator="{ props: tip }">
                        <span v-bind="tip" class="d-inline-flex align-center">
                            <v-btn
                                color="white"
                ...
                <Link class="d-inline-flex text-decoration-none" href="/teachers">
                    <v-btn class="ms-1" color="white" prepend-icon="mdi-view-list" variant="text"> All teachers </v-btn>
                </Link>
            </v-toolbar>
            ...
            <v-data-table
                :headers="headers"
                :items="items"
                class="teachers-archive-table"
                hover
                item-value="id"
                :items-per-page="15"
                :no-data-text="'No teachers in archive.'"
            >
                ...
                <template #header.select>
                    <div class="d-flex justify-center">
                        <v-checkbox-btn
                            class="teachers-archive-checkbox"
...
.teachers-archive-table :deep(.v-data-table__th) {
    font-weight: 600;
}

.teachers-archive-table :deep(.v-data-table-footer) {
    justify-content: flex-start;
}

.teachers-archive-table :deep(.v-data-table-footer__info) {
...
.teachers-archive-checkbox {
    --teachers-cb-size: 36px;
}

.teachers-archive-checkbox :deep(.v-selection-control) {
    min-height: var(--teachers-cb-size);
}

.teachers-archive-checkbox :deep(.v-btn) {
    border: none !important;
    background: transparent !important;
    box-shadow: none !important;
}

.teachers-archive-checkbox :deep(.v-selection-control__wrapper) {
    width: var(--teachers-cb-size);
    height: var(--teachers-cb-size);
}

.teachers-archive-checkbox :deep(.v-selection-control__input) {
    width: var(--teachers-cb-size);
    height: var(--teachers-cb-size);
    border: none !important;
    background: transparent !important;
