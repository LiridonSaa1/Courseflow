...
const previewSrc = computed(() => {
    ...
    return props.student?.profile_photo_url ?? null;
});
...
function applyStudent(s) {
    ...
    form.gender = s.gender ?? null;
    form.profile_photo = null;
    form.status = ['active', 'inactive', 'pending'].includes(s.status) ? s.status : 'active';
    photoFiles.value = [];
...
function submit() {
    if (!canSubmit.value || !props.student?.id) {
        return;
    }
...
watch(
    () => [model.value, props.student],
    () => {
        if (model.value && props.student) {
...
<template>
    <v-navigation-drawer
        v-model="model"
        class="course-create-drawer student-edit-drawer"
        location="end"
        temporary
        width="480"
        scrim
        ...
        <v-toolbar class="align-start py-4" color="primary" flat height="auto">
            ...
            <div class="ms-2 flex min-w-0 flex-1 flex-col gap-1 ps-0">
                ...
                <p class="mb-0 text-body-2 text-white opacity-90">
                    Përditësoni profilin dhe statusin e studentit.
                </p>
            </div>
        </v-toolbar>
        <v-divider />
...
<style>
.student-edit-drawer.v-navigation-drawer--temporary {
...
.v-navigation-drawer.student-edit-drawer ~ .v-navigation-drawer__scrim,
.v-navigation-drawer.student-edit-drawer ~ * .v-navigation-drawer__scrim {
    position: fixed !important;
    inset: 0 !important;
